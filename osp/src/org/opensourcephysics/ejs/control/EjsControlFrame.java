...
public class EjsControlFrame extends ParsedEjsControl implements RootPaneContainer, MainFrame {
  ...
  protected JMenu loadToolsMenu() {
    ...
    try {
      datasetToolClass = Class.forName("org.opensourcephysics.tools.DataTool");
  ...
  protected void loadDisplayMenu() {
    ...
    final JCheckBoxMenuItem textAliasItem = new JCheckBoxMenuItem(EjsRes.getString("EjsControlFrame.Text_check_box"),
      false); // $NON-NLS-1$
  ...
  public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
    ...
    double scalex = pageFormat.getImageableWidth()/(double) mainFrame.getWidth();
    double scaley = pageFormat.getImageableHeight()/(double) mainFrame.getHeight();
  ...
  public void clearDataAndRepaint() {
	  clearData();
	  if(defaultDrawingPanel!=null)defaultDrawingPanel.repaint();
  }
    ...
    if(result==JFileChooser.APPROVE_OPTION) {
      ...
      if(file.exists()) {
        int selected = JOptionPane.showConfirmDialog(
            null,
            EjsRes.getString("EjsControlFrame.ReplaceExisting_dialog")+file.getName()
            +EjsRes.getString("EjsControlFrame.question_mark"), EjsRes.getString(
              "EjsControlFrame.RepalceFile_dialog_message"), // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  public void loadXML(String fileName) {
    ...
    try {
      ...
      System.out.println("[XML file not loaded: "+fileName);
      System.out.println("EjsControlFrame Exception: "+ex);
      //ex.printStackTrace();
      return;
    }
    ...
    if(OSPApplication.class.isAssignableFrom(xml.getObjectClass())) {
      ...
      JOptionPane.showMessageDialog(mainFrame, "\""+fileName+"\" is for "+xml.getObjectClass()+".", // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  public void loadXML(XMLControlElement xml) {
    ...
    if(OSPApplication.class.isAssignableFrom(xml.getObjectClass())) {
      ...
      JOptionPane.showMessageDialog(mainFrame, "XML is for "+xml.getObjectClass()+".", // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                                    "Incorrect XML Object Type", JOptionPane.WARNING_MESSAGE); // $NON-NLS-1$
    }
  }
