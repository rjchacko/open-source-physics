...
public final class ParserSuryono {
  ...
  static private final String constname[] = {"Math.E", "Math.PI"}; // Added by Paco
  ...
  {
    "Math.abs", "Math.acos", "Math.asin", "Math.atan", "Math.ceil", "Math.cos", "Math.exp", "Math.floor", "Math.log", "Math.rint", "Math.round", "Math.sin", "Math.sqrt", "Math.tan", "Math.toDegrees", "Math.toRadians"
  };
  ...
  static public String[] getVariableList(String _expression) { // Added by Paco
    Vector varlist = getVariableList(_expression, new Vector());
    if(varlist.size()<1) {
      return new String[0];
    }
    return(String[]) varlist.toArray(new String[1]);
  }
   ...
   */
  static public Vector getVariableList(String _expression, Vector varlist) { // Added by Paco
    java.util.StringTokenizer tkn = new java.util.StringTokenizer(_expression, "() \t+-*/,<>=&|");
    ...
    while(true) {
      ...
      try {
        ...
        default :
          ...
          } else if((int) code>=FUNCNOPARAM_OFFSET) { // Paco
            stack[++stack_pointer] =                  // This is the other difference for no parameters ! Paco
              builtInFunctionNoParam((int) code-FUNCNOPARAM_OFFSET);
          ...
          } else if((int) code>=CONST_OFFSET) { // Added by Paco for constants
            stack[++stack_pointer] = constvalue[(int) code-CONST_OFFSET];
          } else {
            error = CODE_DAMAGED;
            return Double.NaN;
          }
