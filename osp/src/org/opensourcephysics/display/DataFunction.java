  ...
  public void refreshFunctionData() {
  	...
  	if (varCount != getVarCount()) {
		...
		for (int n = 0; n < len; n++) { // number of data values for each variable
			...
			double val = function.evaluate(fData);
...
//______________________________ private methods ___________________________
  ...
  private String[] getVarNames() {
  	...
  	while (it.hasNext()) {
  		...
  		String name = null;
  		if (names.isEmpty()) {
        name = GUIUtils.removeSubscripting(dataset.getXColumnName());
  			names.add(name);
  		}
      name = GUIUtils.removeSubscripting(dataset.getYColumnName());
			names.add(name);
  	}
   ...
   */
   protected static class Loader extends XMLLoader {
    ...
    public Object loadObject(XMLControl control, Object obj) {
      ...
      // don't load other Dataset properties
      return obj;
    }
  }
}
