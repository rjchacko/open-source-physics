...
public class DatasetCurveFitter extends JPanel {
  ...
  public String getSelectedFitName() {
    for (Iterator it = namedFits.keySet().iterator(); it.hasNext();) {
    	String name = it.next().toString();
    	if (namedFits.get(name) == fit) return name;
    }
    return null;
  }
  ...
  protected void createGUI() {
    ...
    colorButton.setToolTipText(ToolsRes.getString(
    			"DatasetCurveFitter.Button.Color.Tooltip")); //$NON-NLS-1$
    colorButton.addActionListener(new ActionListener() {
		  public void actionPerformed(ActionEvent e) {
		  	JDialog dialog = getColorDialog();
	    	closeButton.setText(ToolsRes.getString(
  					"Button.OK")); //$NON-NLS-1$
	    	dialog.setTitle(ToolsRes.getString(
						"DatasetCurveFitter.Dialog.Color.Title")); //$NON-NLS-1$
    ...
    fitBuilderButton = DataTool.createButton(
    			ToolsRes.getString("DatasetCurveFitter.Button.Define.Text")); //$NON-NLS-1$
    fitBuilderButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
      	...
      	else fitBuilder.customButtons[0].doClick();      		
  ...
  protected FunctionTool getFitBuilder() {
  	if (fitBuilder == null) {
  		newFitButton = new JButton(
  					ToolsRes.getString("DatasetCurveFitter.Button.NewFit.Text")); //$NON-NLS-1$
      newFitButton.setToolTipText(ToolsRes.getString("DatasetCurveFitter.Button.NewFit.Tooltip")); //$NON-NLS-1$
  		newFitButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
        	// create new user function and select it 
        	String name = fitBuilder.getUniqueName(
        				ToolsRes.getString("DatasetCurveFitter.NewFit.Name")); //$NON-NLS-1$
  		...
  		deleteFitButton = new JButton(
  					ToolsRes.getString("DatasetCurveFitter.Button.DeleteFit.Text")); //$NON-NLS-1$
      deleteFitButton.setToolTipText(ToolsRes.getString("DatasetCurveFitter.Button.DeleteFit.Tooltip")); //$NON-NLS-1$
        ...
        public void actionPerformed(ActionEvent e) {
  		...
  		fitBuilder = new FunctionTool(this, 
  					new JButton[] {newFitButton, deleteFitButton});
  ...
  private String getFitName() {
  	for (Iterator it = namedFits.keySet().iterator(); it.hasNext();) {
  		Object key = it.next();
  		if (fit == namedFits.get(key)) return key.toString();
  	}
    return null;
  }
