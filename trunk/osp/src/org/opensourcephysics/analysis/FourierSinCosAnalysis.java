...
public class FourierSinCosAnalysis implements Data {
   ...
   public double[] doAnalysis(double[] x, double[] y, int gutter) {
      fftData = new double[y.length+2*gutter];
      gutterVec = new double[gutter];
      System.arraycopy(y, 0, fftData, gutter, y.length );
      ...
      cosVec[0] *=0.5;  // constant coefficient has factor or 1/2.
      sinVec[0] *=0.5;
      for(int i = 0, nOmega = omega.length; i<nOmega; i++) {
   ...
   public double[] repeatAnalysis(double[] y) {
      ...
      System.arraycopy(y, 0, fftData, gutterVec.length, y.length);
      fft.transform(fftData);                                                                       // Computes the FFT of data leaving the result in fft_pts.
      for(int i = 0, nOmega = omega.length; i<nOmega; i++) {
         double re = fftData[2*i];
         double im = fftData[2*i+1];
   ...
   public java.util.ArrayList getDatasets() {
     ...
     if(fftData==null) {
      ...
      if(realDatasets[0]==null) {
         ...
         realDatasets[0].setXYColumnNames("frequency", "power", "Power Spectrum");
         realDatasets[0].setLineColor(Color.BLACK);
         realDatasets[1] = new Dataset();
         realDatasets[1].setXYColumnNames("frequency", "cos", "Cosine Coefficients");
         realDatasets[1].setLineColor(Color.RED);
         realDatasets[2] = new Dataset();
         realDatasets[2].setXYColumnNames("frequency", "sin", "Sine Coefficients");
         realDatasets[2].setLineColor(Color.BLUE);
      } else {
