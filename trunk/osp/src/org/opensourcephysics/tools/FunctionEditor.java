...
public class FunctionEditor extends JPanel implements PropertyChangeListener {
  ...
  // instance fields
	...
	protected ArrayList sortedObjects = new ArrayList();
	protected HashSet forbiddenNames = new HashSet();
	protected boolean removablesAtTop = false;
	protected Collection circularErrors = new HashSet();
	protected Collection errors = new HashSet();
	protected List evaluate = new ArrayList();
  ...
  public FunctionEditor() {
	 ...
	 */
	public void setObjects(java.util.List newObjects) {
  ...
  public void setExpression(String name, String expression, boolean postEdit) {
	...
	public Object addObject(Object obj, int row, 
		...
		java.util.List newObjects = new ArrayList(objects);
	...
	public void evaluateAll() {
		...
		for (Iterator it = objects.iterator(); it.hasNext();) {
			Object next = it.next();
			String name = getName(next);
			for (Iterator it2 = circularErrors.iterator(); it2.hasNext();) {
		...
		while (!temp.isEmpty()) {
			for (int i = 0; i < temp.size(); i++) {
				...
				String name = getName(next);
				Set references = getReferences(name, null);
   ...
   * @return the set filled with names of referenced functions
	 */
	protected Set getReferences(String name, Set references) {
		if (references == null) {
		...
		if (obj != null) {
			...
			List directReferences = new ArrayList();
			for (Iterator it = sortedObjects.iterator(); it.hasNext();) {
				Object next = it.next();
   ...
   */
  protected void setReferences(Object obj, List referencedObjects) {
  	/** empty block */
  }
		...
		try {
			...
			if (dataString != null) {
				...
				if (control.getObjectClass() == this.getClass()) {
					java.util.List list = control.getPropertyContent();
					XMLProperty selected = (XMLProperty)list.get(0);
					return selected.getChildControls();
				}
			}
  ...
  private String getValidName(String proposedName) {
  	...
  	ArrayList invalid = getInvalidTokens(name);
   ...
   */
  private ArrayList getInvalidTokens(String name) {
